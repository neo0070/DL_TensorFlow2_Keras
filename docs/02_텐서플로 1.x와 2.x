...
1. 텐서플로 1.x의 이해
  ...
  1-2. 상수, 변수, 플레이스홀더와 작업
    ...
    텐서는 기본적으로 n차원 배열임 : 스칼라-0차원, 벡터-1차원, 행렬-2차원 텐서
...
2. 텐서플로 2.x의 이해
  ...
  2-1. 즉시 실행 : 여전히 그래프가 있지만 특별한 세션 인터페이스나 플레이스홀더 없이도 노드를 즉시 정의/변경/실행 가능
  ...
  2-3. 케라스 API : 3가지 프로그래밍 모델
    tf.keras는 다음 3가지 프로그래밍 모델과 함께 더 하이레벨 API를 제공 (특정 요구사항에 따라 세가지를 섞어 사용)
    ① 순차적 API : 90%의 사례에 적합한 매우 우아하고 직관적이며 간결한 모델
    ② 함수적 API : 다중 입력, 다중 출력, 비순차 흐름과의 잔존 연결, 공유, 재사용 가능 계층을 포함해 
  ...
  2-13. 텐서플로 2.x의 효율적인 사용
    ...
    ③ 파이썬 객체를 사용해 변수와 손실을 추적 (tf.get_variable 대신 tf.Variable 사용)
    ④ 데이터 입력에는 tf.data 데이터셋을 사용하고 이 객체를 바로 tf.keras, Model.fit에 제공
      (데이터 조작용 고성능 클래스의 모음을 가질 수 있고 디스크에서 훈련 데이터를 스트림하는 최적의 방법)
    ⑤ 가능하면 tf.layers모듈을 사용해 순차적 또는 함수적 API로 사전 정의된 '레고블록'을 조립 (차선으로 서브클래싱 사용)
      양산(production-ready)모델이 필요한 경우, 특히 여러 GPU/CPU/서버에서 확장하는 경우 추정기 사용 (tf.keras를 추정기로 변환 고려)
    ⑥ GPU/CPU/여러 플랫폼으로 분산 전략 사용을 고려

----------------------------------------------------------------------------------------------------------------------
3. 텐서플로 2.x 생태계
  2.x는 핵심 학습 엔진 외에도 자유롭게 사용할 수 있는 다양한 도구 모음이 있는 풍부한 생태계다.
  - 도구들 : 텐서플로.js, 텐서플로 라이트, 텐서플로 Hub, 텐서플로 익스텐디드, 텐서보드, Sonnet, 텐서보드 Federated, 텐서보드 Probability, 
            텐서보드 Playground, Accelerated Linear Algebra, MLPerf, Colab, 텐서플로 Datasets
    ※ 이 책에서는 Colab, 텐서보드, Hub, 라이트를 사용

  - 언어 바인딩 : 파인썬은 텐서플로가 선택한 언어임
    . 자바스크립트, C++, 자바, Go, Swift 포함 다른 언어와 바인딩 가능 (커뮤니티 : C#, Haskell, Julia, Ruby, Rust, Scala 등)

-----------------------------------------------------------------------------------------------------------------------
4. 케라스 또는 tf.keras 중 어떤 것?
  텐서플로와 함께 백엔드로 케라스 or tf.keras의 API 사용? (케라스:tf.keras엔드포인트 구현 X, tf.keras: 다양한 벡엔드 지원 X)
  → tf.keras 사용을 권장
    . 텐서플로의 향상된 기능(즉시 실행, TPU훈련을 포함한 기본 분산 훈련 지원, 텐서플로 SavedModel 교환 형식 지원) 지원
    . 케라스는 독립적인 오픈소스 프로젝트이며, 그 개발이 텐서플로에 종속적되지 않음
    ※ 케라스도 구글 텐서플로, MS CNTK, 아마존 MXnet, Theano포함 여러 백엔드에서 실행할 수 있는 이식성이 뛰어난 코드 작성이 유용함

  - 각 경연 대회에서 캐글(Kaggle) 상위 5개 팀이 사용하는 주요 머신러닝 소프트웨어 도구
    케라스 > LightGBM > XBGoard > 파이토치 > 텐서플로(비케라스) > Sci-kit Learn > Fastai > Caffe 
  
--------------------------------------------------------------------------------------------------------------------------
● 요약
  텐서플로는 2.0은 "훈련"과 "서비스"라는 두가지 주요 부분으로 구성된 풍부한 개발 생태계임
  - 훈련 : 하이레벨 라이브러리(tf.data), 로우레벨 라이브러리(tf.*), 미리 훈련시킨 모델(tf.Hub)을 포함해 모델 작성용 라이브러리
          집합인 데이터셋(tft.data)를 처리하는 데 필요한 라이브러리 집합으로 구성되며,
          분산 전략을 통해 CPU/GPU/TPU에서 이뤄질 수 있고, 적절한 라이브러리를 사용해 결과를 저장할 수 있음
  - 서비스 : On-prem/Cloud/안드로이드/iOS/RaspberryPie/자바스크립트를 지원하는 브라우저/Node.js 를 포함함 여러 플랫폼에서 이뤄질수 있음
             또한, 파이썬/C/C#/자바/스위프트/R 등을 포함한 많은 언어 바인딩을 지원
  - 텐서플로 2.0 아키텍처 요약
    ① tf.data : 매우 효율적인 방법으로 모델을 로드
    ② tf.keras, 추정기 : 하이레벨 라이브러리지만 tf.* 로우레벨 라이브러리를 통해 여전히 1.x의 기능에 접근 가능
    ③ tf.hub : 즉시 사용할 수 있는 사전 훈련된 모델 모음
    ④ 분산 전략 : CPU/GPU/TPU에서 훈련을 실행
    ⑤ 저장된 모델은 여러 플랫폼에서 제공될 수 있다
